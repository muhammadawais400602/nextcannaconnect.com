...
export default function ExtractionProfileTabs({ company, similar }: { company: Company; similar: Company[] }) {
  const stats = [
    ...
    company.turnaroundTime ? { label: "Turnaround", value: company.turnaroundTime } : null,
    company.teamSize ? { label: "Team", value: company.teamSize } : null,
  ].filter(Boolean) as { label: string; value: string }[];

  const hasMethods = (company.serviceTags?.length ?? 0) > 0;
  const hasProducts = (company.products?.length ?? 0) > 0;
  const hasCerts = (company.certifications?.length ?? 0) > 0;
  const hasFaqs = (company.faqs?.length ?? 0) > 0;
  const isElite = company.tier === "elite";

  const tabs = [
    { id: "overview", label: "Overview" },
    hasMethods ? { id: "methods", label: "Extraction Methods", locked: !isElite } : null,
    hasProducts ? { id: "products", label: "Output Products", locked: !isElite } : null,
    hasCerts ? { id: "certifications", label: "Certifications" } : null,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
      ...
      {active === "methods" && hasMethods && (
        <section style={{ background: "white", border: `1px solid ${PARCHMENT}`, borderRadius: "0.5rem", padding: "32px" }}>
          <h2 style={{ fontSize: "12px", fontWeight: 600, color: VARIANT, textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 16px" }}>Extraction Methods</h2>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "12px" }}>
            {company.serviceTags!.map((tag) => (
              <div key={tag} style={{ display: "flex", alignItems: "center", gap: "8px", background: "#fbf9f8", border: `1px solid ${PARCHMENT}`, borderRadius: "0.5rem", padding: "10px 16px", fontSize: "14px", fontWeight: 500, color: INK }}>
                <span className="material-symbols-outlined" style={{ fontSize: "16px", color: SAGE }}>science</span> {tag}
              </div>
            ))}
          </div>
        </section>
      ...
      {active === "products" && hasProducts && (
        <section style={{ background: "white", border: `1px solid ${PARCHMENT}`, borderRadius: "0.5rem", padding: "32px" }}>
          <h2 style={{ fontSize: "12px", fontWeight: 600, color: VARIANT, textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 16px" }}>Output Products</h2>
                ...
                <div style={{ height: "128px", borderRadius: "0.25rem", marginBottom: "16px", overflow: "hidden", background: c.logoColor + "22" }}>
                  {c.bannerImageUrl ? (
                    ...
                    <div style={{ width: "100%", height: "100%", background: `linear-gradient(135deg, ${c.logoColor}33, ${c.logoColor}11)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span className="material-symbols-outlined" style={{ fontSize: "32px", color: c.logoColor, opacity: 0.4 }}>science</span>
                    </div>
                  )}
                </div>
