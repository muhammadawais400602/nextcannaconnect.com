...
export default function ManufacturersProfileTabs({ company, similar }: { company: Company; similar: Company[] }) {
  const stats = [
    ...
    company.serviceArea ? { label: "States Shipped", value: company.serviceArea } : null,
    company.minOrderQty ? { label: "MOQ", value: company.minOrderQty } : null,
  ...
  const tabs = [
    ...
    hasProducts ? { id: "products", label: "Products & Catalog", locked: !isElite } : null,
                ...
                <div style={{ height: "128px", borderRadius: "0.25rem", marginBottom: "16px", overflow: "hidden", background: c.logoColor + "22" }}>
                  {c.bannerImageUrl ? (
                    ...
                    <div style={{ width: "100%", height: "100%", background: `linear-gradient(135deg, ${c.logoColor}33, ${c.logoColor}11)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span className="material-symbols-outlined" style={{ fontSize: "32px", color: c.logoColor, opacity: 0.4 }}>factory</span>
                    </div>
                  )}
                </div>
