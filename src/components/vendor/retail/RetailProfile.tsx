...
import RetailProfileTabs from "./RetailProfileTabs";
...
export default function RetailProfile({ company, similar }: Props) {
  ...
  const pills = [
    ...
    company.delivery === "Yes" || company.delivery === "Only"
      ? { icon: "local_shipping", label: "Wholesale Delivery" }
      : null,
  ...
  return (
    <div style={{ background: "#FBF9F8", fontFamily: "'Inter', sans-serif", color: "#1b1c1b", paddingTop: "70px" }}>
      <style>{`
        .rp-container { max-width: 1280px; margin: 0 auto; padding-left: 20px; padding-right: 20px; }
        @media (min-width: 768px) { .rp-container { padding-left: 64px; padding-right: 64px; } }
        .rp-grid { display: grid; grid-template-columns: 1fr; gap: 24px; }
        @media (min-width: 1024px) { .rp-grid { grid-template-columns: 8fr 4fr; } }
        .rp-identity { display: flex; flex-direction: column; gap: 24px; }
        @media (min-width: 768px) { .rp-identity { flex-direction: row; align-items: center; justify-content: space-between; } }
        .rp-sidebar { position: static; }
        @media (min-width: 1024px) { .rp-sidebar { position: sticky; top: 96px; align-self: start; } }
      `}</style>

      {/* Breadcrumb */}
      <div style={{ padding: "16px 0" }}>
        <div className="rp-container" style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "12px", fontWeight: 600, color: VARIANT, flexWrap: "wrap" }}>
          ...
          <span className="material-symbols-outlined" style={{ fontSize: "16px" }}>chevron_right</span>
          <Link href="/directory/retail-dispensary" style={{ color: VARIANT, textDecoration: "none" }}>Retail &amp; Dispensary</Link>
          <span className="material-symbols-outlined" style={{ fontSize: "16px" }}>chevron_right</span>
      ...
      <div style={{ position: "relative", width: "100%", height: "288px" }}>
        ...
        <div className="rp-container" style={{ position: "relative", height: "100%" }}>
          <div style={{ position: "absolute", top: "24px", left: "20px" }}>
            <Link href="/directory/retail-dispensary" style={{ display: "flex", alignItems: "center", gap: "8px", color: "white", fontSize: "14px", fontWeight: 500, background: "rgba(0,0,0,0.2)", padding: "8px 16px", borderRadius: "0.5rem", backdropFilter: "blur(4px)", textDecoration: "none" }}>
              <span className="material-symbols-outlined" style={{ fontSize: "20px" }}>arrow_back</span>
              <span>Back to Retail &amp; Dispensary</span>
            </Link>
          </div>
        </div>
      </div>
      ...
      {/* Main */}
      <main className="rp-container" style={{ position: "relative", paddingBottom: "96px" }}>
        {/* Identity card (overlapping) */}
        <div className="rp-identity" style={{ background: "white", border: `1px solid ${PARCHMENT}`, borderRadius: "0.5rem", boxShadow: "0 10px 25px -5px rgba(0,51,32,0.1)", padding: "32px", marginTop: "-96px", position: "relative", zIndex: 20, marginBottom: "48px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px", minWidth: "200px" }}>
            <a href="#get-in-touch" style={{ background: DF, color: "white", borderRadius: "0.25rem", fontSize: "14px", fontWeight: 500, padding: "12px 24px", textAlign: "center", textDecoration: "none" }}>Contact Dispensary</a>
            <a href="#get-in-touch" style={{ border: `1px solid ${DF}`, color: DF, borderRadius: "0.25rem", fontSize: "14px", fontWeight: 500, padding: "12px 24px", textAlign: "center", textDecoration: "none" }}>Request Wholesale Info</a>
          </div>
        </div>
        ...
        <div className="rp-grid">
          {/* Left */}
          <div>
            <RetailProfileTabs company={company} similar={similar} />
          </div>
      ...
      <div style={{ width: "100%", background: DF, padding: "64px 0", textAlign: "center" }}>
        <div className="rp-container">
          <h2 style={{ fontSize: "clamp(24px, 4vw, 32px)", fontWeight: 600, color: "white", margin: "0 0 24px" }}>Are you a wholesale buyer or brand?</h2>
          <Link href="/signup" style={{ background: "white", color: DF, borderRadius: "0.25rem", fontSize: "14px", fontWeight: 700, padding: "12px 32px", textDecoration: "none", display: "inline-block" }}>Create Buyer Account</Link>
        </div>
      </div>
    </div>
  );
