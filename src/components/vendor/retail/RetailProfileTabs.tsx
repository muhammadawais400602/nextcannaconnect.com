...
export default function RetailProfileTabs({ company, similar }: { company: Company; similar: Company[] }) {
  const stats = [
    ...
    company.locationsCount ? { label: "Locations", value: String(company.locationsCount) } : null,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
      ...
      {active === "licenses" && hasLicenses && (
        <section style={{ background: "white", border: `1px solid ${PARCHMENT}`, borderRadius: "0.5rem", padding: "32px" }}>
          <h2 style={{ fontSize: "12px", fontWeight: 600, color: VARIANT, textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 8px" }}>Licensing &amp; Operations</h2>
          <div>
            {company.licenseNumber && <DetailRow label="License Number" value={company.licenseNumber} />}
            {company.licenseType && <DetailRow label="License Type" value={company.licenseType} />}
            {company.delivery && <DetailRow label="Delivery" value={company.delivery} />}
            {company.hours && <DetailRow label="Hours" value={company.hours} />}
            ...
            {company.metrcIntegrated && <DetailRow label="Metrc Integrated" value="Yes" />}
          </div>
        </section>
      )}
                ...
                <div style={{ height: "128px", borderRadius: "0.25rem", marginBottom: "16px", overflow: "hidden", background: c.logoColor + "22" }}>
                  {c.bannerImageUrl ? (
                    ...
                    <div style={{ width: "100%", height: "100%", background: `linear-gradient(135deg, ${c.logoColor}33, ${c.logoColor}11)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span className="material-symbols-outlined" style={{ fontSize: "32px", color: c.logoColor, opacity: 0.4 }}>storefront</span>
                    </div>
                  )}
                </div>
