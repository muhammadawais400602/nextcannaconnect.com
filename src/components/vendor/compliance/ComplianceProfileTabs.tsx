...
export default function ComplianceProfileTabs({ company, similar }: { company: Company; similar: Company[] }) {
  const stats = [
    company.foundedYear ? { label: "Founded", value: String(company.foundedYear) } : null,
    company.yearsInCannabis ? { label: "Years in Cannabis Law", value: String(company.yearsInCannabis) } : null,
    company.serviceArea ? { label: "States Practiced", value: company.serviceArea } : null,
    company.teamSize ? { label: "Team", value: company.teamSize } : null,
  ].filter(Boolean) as { label: string; value: string }[];

  const hasCredentials = (company.credentials ?? []).length > 0 || (company.certifications ?? []).length > 0;
  const hasPracticeAreas = (company.serviceTags ?? []).length > 0;
  const hasCaseStudies = (company.caseStudies ?? []).length > 0;
  const hasFaqs = (company.faqs?.length ?? 0) > 0;
  const isElite = company.tier === "elite";

  const tabs = [
    { id: "overview", label: "Overview" },
    hasCredentials ? { id: "credentials", label: "Credentials" } : null,
    hasPracticeAreas ? { id: "practice", label: "Practice Areas", locked: !isElite } : null,
    hasCaseStudies ? { id: "cases", label: "Notable Cases", locked: !isElite } : null,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
      ...
      {active === "credentials" && hasCredentials && (
        <section style={{ background: "white", border: `1px solid ${PARCHMENT}`, borderRadius: "0.5rem", padding: "32px" }}>
          <h2 style={{ fontSize: "12px", fontWeight: 600, color: VARIANT, textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 16px" }}>Credentials &amp; Certifications</h2>
          {(company.credentials ?? []).length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", marginBottom: (company.certifications ?? []).length > 0 ? "24px" : 0 }}>
              {company.credentials!.map((cr) => (
                <span key={cr} style={{ display: "flex", alignItems: "center", gap: "6px", background: "rgba(187,238,209,0.15)", color: DF, fontSize: "13px", fontWeight: 600, padding: "8px 14px", borderRadius: "0.5rem", border: `1px solid ${SAGE}44` }}>
                  <span className="material-symbols-outlined" style={{ fontSize: "16px", color: SAGE }}>workspace_premium</span> {cr}
                </span>
              ))}
            </div>
          )}
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                {company.certifications!.map((cert) => (
                  <DetailRow key={cert} label={cert} value="Active" />
                ))}
              </div>
            </div>
          )}
      ...
      {active === "practice" && hasPracticeAreas && (
        <section style={{ background: "white", border: `1px solid ${PARCHMENT}`, borderRadius: "0.5rem", padding: "32px" }}>
          <h2 style={{ fontSize: "12px", fontWeight: 600, color: VARIANT, textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 16px" }}>Practice Areas</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {company.serviceTags!.map((tag) => (
              <div key={tag} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px 0", borderBottom: `1px solid ${PARCHMENT}` }}>
                <span className="material-symbols-outlined" style={{ fontSize: "20px", color: SAGE }}>gavel</span>
        ...
        <section style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          {company.caseStudies!.map((cs) => (
            <div key={cs.title} style={{ background: "white", border: `1px solid ${PARCHMENT}`, borderRadius: "0.5rem", padding: "24px" }}>
              <h3 style={{ fontSize: "16px", fontWeight: 600, color: INK, margin: "0 0 8px" }}>{cs.title}</h3>
                ...
                <div style={{ height: "128px", borderRadius: "0.25rem", marginBottom: "16px", overflow: "hidden", background: c.logoColor + "22" }}>
                  {c.bannerImageUrl ? (
                    ...
                    <div style={{ width: "100%", height: "100%", background: `linear-gradient(135deg, ${c.logoColor}33, ${c.logoColor}11)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span className="material-symbols-outlined" style={{ fontSize: "32px", color: c.logoColor, opacity: 0.4 }}>gavel</span>
                    </div>
                  )}
                </div>
