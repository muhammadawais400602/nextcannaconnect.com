...
function FacilityCard({ company }: { company: Company }) {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "24px" }}>
          {(company.serviceTags ?? []).length > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px", borderBottom: `1px solid ${PARCHMENT}`, paddingBottom: "8px" }}>
              <span style={{ color: VARIANT }}>Method</span>
              <span style={{ fontWeight: 500 }}>{company.serviceTags![0]}</span>
            </div>
          )}
          {company.facilitySize && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px", borderBottom: `1px solid ${PARCHMENT}`, paddingBottom: "8px" }}>
              <span style={{ color: VARIANT }}>Facility</span>
              <span style={{ fontWeight: 500 }}>{company.facilitySize}</span>
            </div>
          )}
          {company.turnaroundTime && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px", borderBottom: company.delivery ? `1px solid ${PARCHMENT}` : "none", paddingBottom: "8px" }}>
              <span style={{ color: VARIANT }}>Turnaround</span>
              <span style={{ fontWeight: 500 }}>{company.turnaroundTime}</span>
            </div>
          )}
          {company.delivery && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px" }}>
              <span style={{ color: VARIANT }}>White Label</span>
...
export default function ExtractionListingsGrid({ companies }: { companies: Company[] }) {
  ...
  const methods = useMemo(() => [...new Set(companies.flatMap((c) => c.serviceTags ?? []))].sort(), [companies]);

  const filtered = useMemo(() => {
    let result = companies.filter((c) => {
      if (location && c.location.state !== location) return false;
      if (method && !(c.serviceTags ?? []).includes(method)) return false;
      return true;
    });
    if (sort === "name") result = [...result].sort((a, b) => a.name.localeCompare(b.name));
    if (sort === "newest") result = [...result].sort((a, b) => (b.foundedYear ?? 0) - (a.foundedYear ?? 0));
    return result;
  }, [companies, location, method, sort]);
    ...
    <>
      <div style={{ background: "white", borderTop: `1px solid ${PARCHMENT}`, borderBottom: `1px solid ${PARCHMENT}`, position: "sticky", top: "70px", zIndex: 40 }}>
        <div className="cat-container" style={{ padding: "16px 0", display: "flex", flexWrap: "wrap", alignItems: "center", gap: "12px" }}>
          ...
          <select value={method} onChange={(e) => { setMethod(e.target.value); setVisible(PAGE_SIZE); }} style={selectStyle}>
            <option value="">All Extraction Methods</option>
        ...
        <div className="cat-container">
          {shown.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 20px", color: VARIANT }}>
              ...
              <p style={{ fontSize: "16px", marginTop: "12px" }}>No extraction facilities match your filters yet.</p>
            </div>
          ) : (
            <div className="cat-grid">
              {shown.map((c) => <FacilityCard key={c.slug} company={c} />)}
              ...
              <div style={{ background: DF, borderRadius: "0.75rem", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", padding: "32px" }}>
                <span className="material-symbols-outlined" style={{ color: SAGE, fontSize: "48px", marginBottom: "16px" }}>science</span>
                <h3 style={{ fontSize: "24px", fontWeight: 600, color: "#FBF9F8", margin: "0 0 8px" }}>Are you an extraction facility?</h3>
                <p style={{ fontSize: "15px", color: "rgba(251,249,248,0.8)", margin: "0 0 24px" }}>List your operation and reach verified cannabis buyers.</p>
                <Link href="/signup" style={{ fontSize: "14px", fontWeight: 500, background: SAGE, color: DF, padding: "12px 24px", borderRadius: "0.25rem", textDecoration: "none", width: "100%", boxSizing: "border-box" }}>List Your Facility</Link>
              </div>
            </div>
          )}
