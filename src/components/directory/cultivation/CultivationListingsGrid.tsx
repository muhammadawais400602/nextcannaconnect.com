      ...
      <div style={{ height: "192px", position: "relative", background: company.logoColor + "22" }}>
        {company.bannerImageUrl ? (
          ...
          <div style={{ width: "100%", height: "100%", background: `linear-gradient(135deg, ${company.logoColor}33, ${company.logoColor}11)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span className="material-symbols-outlined" style={{ fontSize: "44px", color: company.logoColor, opacity: 0.4 }}>nature</span>
          </div>
        )}
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "16px", gap: "12px" }}>
          ...
          <div style={{ width: "48px", height: "48px", background: "#efedec", borderRadius: "50%", border: `1px solid ${PARCHMENT}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
            {company.logoUrl ? (
              ...
              <span className="material-symbols-outlined" style={{ color: "#c0c9c1" }}>nature</span>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "24px" }}>
          {company.serviceArea && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px", borderBottom: `1px solid ${PARCHMENT}`, paddingBottom: "8px" }}>
              <span style={{ color: VARIANT }}>Canopy Size</span>
              <span style={{ fontWeight: 500 }}>{company.serviceArea}</span>
            </div>
          )}
          {company.transportType && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px", borderBottom: `1px solid ${PARCHMENT}`, paddingBottom: "8px" }}>
              <span style={{ color: VARIANT }}>Grow Type</span>
              <span style={{ fontWeight: 500 }}>{company.transportType}</span>
            </div>
          )}
          {company.teamSize && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px", borderBottom: company.delivery ? `1px solid ${PARCHMENT}` : "none", paddingBottom: "8px" }}>
              <span style={{ color: VARIANT }}>Team Size</span>
              <span style={{ fontWeight: 500 }}>{company.teamSize}</span>
            </div>
          )}
          {company.delivery && (
...
export default function CultivationListingsGrid({ companies }: { companies: Company[] }) {
  ...
  const growTypes = useMemo(() => [...new Set(companies.flatMap((c) => c.serviceTags ?? []))].sort(), [companies]);

  const filtered = useMemo(() => {
    let result = companies.filter((c) => {
      if (location && c.location.state !== location) return false;
      if (growType && !(c.serviceTags ?? []).includes(growType)) return false;
      return true;
    });
    if (sort === "name") result = [...result].sort((a, b) => a.name.localeCompare(b.name));
    if (sort === "newest") result = [...result].sort((a, b) => (b.foundedYear ?? 0) - (a.foundedYear ?? 0));
    return result;
  }, [companies, location, growType, sort]);
    ...
    <>
      <div style={{ background: "white", borderTop: `1px solid ${PARCHMENT}`, borderBottom: `1px solid ${PARCHMENT}`, position: "sticky", top: "70px", zIndex: 40 }}>
        <div className="cat-container" style={{ padding: "16px 0", display: "flex", flexWrap: "wrap", alignItems: "center", gap: "12px" }}>
          ...
          <select value={growType} onChange={(e) => { setGrowType(e.target.value); setVisible(PAGE_SIZE); }} style={selectStyle}>
            <option value="">All Grow Types</option>
            {growTypes.map((g) => <option key={g} value={g}>{g}</option>)}
          </select>
        ...
        <div className="cat-container">
          {shown.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 20px", color: VARIANT }}>
              <span className="material-symbols-outlined" style={{ fontSize: "48px", color: "#c0c9c1" }}>nature</span>
              <p style={{ fontSize: "16px", marginTop: "12px" }}>No cultivators match your filters yet.</p>
            </div>
          ) : (
            <div className="cat-grid">
              {shown.map((c) => <CultivatorCard key={c.slug} company={c} />)}
              ...
              <div style={{ background: DF, borderRadius: "0.75rem", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", padding: "32px" }}>
                <span className="material-symbols-outlined" style={{ color: SAGE, fontSize: "48px", marginBottom: "16px" }}>nature</span>
                <h3 style={{ fontSize: "24px", fontWeight: 600, color: "#FBF9F8", margin: "0 0 8px" }}>Are you a licensed cultivator?</h3>
                <p style={{ fontSize: "15px", color: "rgba(251,249,248,0.8)", margin: "0 0 24px" }}>Join the verified network of compliant cannabis grows.</p>
                <Link href="/signup" style={{ fontSize: "14px", fontWeight: 500, background: SAGE, color: DF, padding: "12px 24px", borderRadius: "0.25rem", textDecoration: "none", width: "100%", boxSizing: "border-box" }}>List Your Grow</Link>
              </div>
            </div>
          )}
