...
function ProviderCard({ company }: { company: Company }) {
      ...
      <div style={{ height: "192px", position: "relative", background: company.logoColor + "22" }}>
        {company.bannerImageUrl ? (
          ...
          <div style={{ width: "100%", height: "100%", background: `linear-gradient(135deg, ${company.logoColor}33, ${company.logoColor}11)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span className="material-symbols-outlined" style={{ fontSize: "44px", color: company.logoColor, opacity: 0.4 }}>account_balance</span>
          </div>
        )}
        {(verified || company.certifications?.length) && (
          <div style={{ position: "absolute", top: "16px", left: "16px", background: SAGE, color: DF, fontSize: "12px", fontWeight: 600, padding: "4px 8px", borderRadius: "0.25rem", boxShadow: "0 1px 2px rgba(0,0,0,0.1)", display: "flex", alignItems: "center", gap: "4px" }}>
            <span className="material-symbols-outlined" style={{ fontSize: "14px" }}>verified</span> {company.certifications?.[0] || "Verified"}
          </div>
        )}
      </div>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "16px", gap: "12px" }}>
          ...
          <div style={{ width: "48px", height: "48px", background: "#efedec", borderRadius: "50%", border: `1px solid ${PARCHMENT}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
            {company.logoUrl ? (
              ...
              <span className="material-symbols-outlined" style={{ color: "#c0c9c1" }}>account_balance</span>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "24px" }}>
          {company.licenseNumber && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px", borderBottom: `1px solid ${PARCHMENT}`, paddingBottom: "8px" }}>
              <span style={{ color: VARIANT }}>License (NMLS)</span>
              <span style={{ fontWeight: 500 }}>{company.licenseNumber}</span>
            </div>
          )}
          {(company.serviceTags ?? []).length > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px", borderBottom: `1px solid ${PARCHMENT}`, paddingBottom: "8px" }}>
              <span style={{ color: VARIANT }}>Service Type</span>
              <span style={{ fontWeight: 500 }}>{company.serviceTags![0]}</span>
            </div>
          )}
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px", borderBottom: company.leadTime ? `1px solid ${PARCHMENT}` : "none", paddingBottom: "8px" }}>
            <span style={{ color: VARIANT }}>States Served</span>
            <span style={{ fontWeight: 500, color: SAGE }}>{company.serviceArea || (isVerified(company) ? company.location.state : "—")}</span>
          </div>
          {company.leadTime && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "16px" }}>
              <span style={{ color: VARIANT }}>Avg Funding Time</span>
              <span style={{ fontWeight: 500 }}>{company.leadTime}</span>
            </div>
          )}
        </div>
...
export default function FinanceListingsGrid({ companies }: { companies: Company[] }) {
  ...
  const filtered = useMemo(() => {
    let result = companies.filter((c) => {
      ...
      if (serviceType && !(c.serviceTags ?? []).includes(serviceType)) return false;
      return true;
    });
    if (sort === "name") result = [...result].sort((a, b) => a.name.localeCompare(b.name));
    if (sort === "newest") result = [...result].sort((a, b) => (b.foundedYear ?? 0) - (a.foundedYear ?? 0));
    return result;
  }, [companies, location, serviceType, sort]);
        ...
        <div className="cat-container">
          {shown.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 20px", color: VARIANT }}>
              <span className="material-symbols-outlined" style={{ fontSize: "48px", color: "#c0c9c1" }}>account_balance</span>
              <p style={{ fontSize: "16px", marginTop: "12px" }}>No financial providers match your filters yet.</p>
            </div>
          ) : (
            <div className="cat-grid">
              {shown.map((c) => <ProviderCard key={c.slug} company={c} />)}
              ...
              <div style={{ background: DF, borderRadius: "0.75rem", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", padding: "32px" }}>
                <span className="material-symbols-outlined" style={{ color: SAGE, fontSize: "48px", marginBottom: "16px" }}>account_balance</span>
                <h3 style={{ fontSize: "24px", fontWeight: 600, color: "#FBF9F8", margin: "0 0 8px" }}>Are you a cannabis financial services provider?</h3>
                <p style={{ fontSize: "15px", color: "rgba(251,249,248,0.8)", margin: "0 0 24px" }}>Join the verified directory and connect with licensed operators who need trusted financial partners.</p>
                <Link href="/signup" style={{ fontSize: "14px", fontWeight: 500, background: SAGE, color: DF, padding: "12px 24px", borderRadius: "0.25rem", textDecoration: "none", width: "100%", boxSizing: "border-box" }}>List Your Business</Link>
              </div>
            </div>
          )}
